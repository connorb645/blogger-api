...
struct DocumentController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        let documentRoute = routes.grouped("documents")
    ...
    func upload(req: Request) async throws -> String {
        ...
        guard let byteBuffer = oByteBuffer else {
            throw Abort(.noContent, reason: "No data found")
        }
    ...
    func fileExtension(for headers: HTTPHeaders) throws -> String {
        ...
        case .jpeg:
            return "jpg"
        default:
            throw Abort(.unsupportedMediaType, reason: "Unsupported content type \(contentType)")
        }
